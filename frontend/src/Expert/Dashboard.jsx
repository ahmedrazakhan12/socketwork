...
function Dashboard() {
  const { authUser, authUserSubscriptions, authUserActiveSubscription ,userDataAuth } =
    useAppContext();
  ...
  // console.log( )
  useEffect(() => {
    console.log(prevRoute, " prev-rou");
    // setOpen(true);
    getLoggedinUserData(localStorage.getItem("token")).then((_) => {
      ...
      console.log(_?.data?.user?.user_appointments, " ");

      // ask for becoming an expert
      if (userType != "expert") {
        // const wannaBeExpert = confirm('Do You Want To Be An Expert?');
        const wannaBeExpert = true;
        setConfirmExpert(wannaBeExpert ? "expert" : "user");
      }
    });
  }, []);
  ...
  function isItEligableForLiveStream() {
    const liveStreamsCompeted = authUserActiveSubscription.live_stream_count;
    const totalLiveStreamCounts =
      authUserActiveSubscription?.subscription_package?.live_stream_counts;

    return liveStreamsCompeted < totalLiveStreamCounts;
  }
  return (
    <div className="min-h-screen bg-gray-100">
        ...
        <Box sx={style}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Typography id="modal-modal-title" variant="h4" component="h2" style={{textTransform: "capitalize"}}>
              Welcom To Zyacom {authUser?.user?.name || ""}!{" "}
            </Typography>
          ...
          <Typography
            ...
            sx={{ mt: 1, mb: 2 }}
          >
            a platform that is dedicated to empowering both users and experts
            alike. We are delighted to have you here and look forward to
            providing you with a pleasant and fulfilling experience.{" "}
          </Typography>
          <Button
      ...
      <section className="tk-scetiondb">
        <div className="container">
          ...
          {/* <span style={{ fontSize: "32px",fontWeight:'bold',textTransform:'capitalize' }} className="m-1" >Welcome</span> */}
          {/* <span style={{ fontSize: "32px",fontWeight:'bold' , textTransform:'capitalize' }}  className="m-1">{userDataAuth?.name}!</span> */}
          <h2 style={{ fontSize: "32px",fontWeight:'bold' , textTransform:'capitalize' }}>Welcome  {userDataAuth?.name}!</h2>
            ...
            <div className="col-xl-4 col-lg-5 col-md-12">
              <div className="tk-seller-aside">
                <ul className="tk-aside-list">
                  ...
                  <li className="w-100">
                    <div className="tk-list-detail tk-bg-lightwheat">
                      <div className="tk-list-name">
                  ...
                  {/* <li className="w-100">
                    <div className="tk-list-detail tk-bg-lightwheat">
                      <div className="tk-list-name">
                        <div className="tk-list-icon tk-list-icon-yellow">
                          <i className="text-dark bi bi-camera-video-fill"></i>
                        </div>
                        <div className="tk-list-heading" id="available_balance">
                          <h5>{authUserActiveSubscription?.subscription_package ? authUserActiveSubscription?.subscription_package?.live_stream_counts : '0'}</h5>
                          <div className="tk-income-detail">
                            <p>Livestream Counts</p>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="row">
              ...
              <div className="col-lg-4 gy-4">
                <div className="tk-paymentways tk-project-box">
                  ...
                  <Link
                    to="/add-wallet"
                    className="tk-withdraw-button tk-withdrawamt"
                  >
                    <b>Withdraw now </b>
                  </Link>
                </div>
                {authUserActiveSubscription &&
                  authUserSubscriptions?.userProfilePromotePackage && (
                    <div className="tk-paymentways tk-project-box">
                      <div className=" tk-projectboxinner">
                        <ActiveSubscriptionCard
                          profileData={
                            authUserSubscriptions?.userPromotePackageDuration
                          }
                        />
                      </div>
                    </div>
